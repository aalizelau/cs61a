...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.can_contain(existing_ant):
                self.store_ant(existing_ant)
                place.ant = self
            elif existing_ant.can_contain(self):
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while current_place is not None: #check is place is valid 
            if current_place.ant is not None: #check if there is any ant 
